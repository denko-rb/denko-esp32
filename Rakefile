DENKO_ESP32_ROOT = File.dirname(File.expand_path(__FILE__))
MRUBY_ROOT = File.join(DENKO_ESP32_ROOT, "components/picoruby-esp32/picoruby")
...
%w[esp32 esp32s3 esp32c3 esp32c6 esp32h2 esp32p4].each do |name|
...
{ microruby: :mruby }.each do |name, vm|
  namespace name do
...
task :flash_factory do
  ...
  sh "esptool.py -b 460800 write_flash 0x10000 build/denko-esp32.bin"
end
...
task :clean do
  sh "idf.py clean"
  FileUtils.cd MRUBY_ROOT do
    %w[xtensa-denko riscv-denko].each do |mruby_config|
      sh "MRUBY_CONFIG=#{DENKO_ESP32_ROOT}/components/picoruby-esp32/build_config/#{mruby_config}.rb rake clean"
    end
  end
end
...
task :gen_wifi_config do
  ...
  output_path = File.join(DENKO_ESP32_ROOT, "storage/etc/network/wifi.yml")
